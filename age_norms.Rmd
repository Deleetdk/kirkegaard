...
test_HS(resid = resid(lm(score ~ age, data = data, na.action = na.exclude)), x = data$age)
...
#visual confirmation by plotting centiles
GG_quantiles(data, "age", "score", quantiles = c(.05, .25, .5, .75, .95))
GG_quantiles(data, "age", "IQ", quantiles = c(.05, .25, .5, .75, .95))
